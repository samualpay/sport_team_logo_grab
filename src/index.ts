...
async function main() {
  try {
    let footballTeams = await Promise.all(
      footballUrls.map(async (url) => {
        return {
          images: await new Grab310win(url.url).findImages(),
          folder: url.folder,
        };
      })
    );
    let nbaTeams = {
      images: await new GrabNBASina(nbaUrl).findImages(),
      folder: "NBA",
    };
    let cbaTeams = {
      images: await new GrabCBASina(cbaUrl).findImages(),
      folder: "CBA",
    };
